...
Fin = 0.5
# Create volumes for
controller = ctl.tf([100],[1])
tfTank1 = ctl.tf([1], [np.pi*r1**2, k])
tfTank1 = ctl.feedback(tfTank1, controller)
print(tfTank1)
volume_set = 50
_, volume_Tank1 = ctl.step_response(tfTank1*45, T=t)
volume_Tank1 = volume_Tank1 + volume_set
volume_set_array = np.ones((len(t), 1))*volume_set
############################ ANIMATION #################################
frame_amount=len(t)
...
tnk_1,=ax1.plot([],[],'blue',linewidth=1,label='Tank 1')
tnk_set,=ax1.plot(t, volume_set_array,'red',linewidth=1,label='Set Point')
# ax1.add_patch(tnk_set)
plt.xlim(t0,t_end)
plt.ylim(volume_i,volume_f)
